...
| 分類 | 範囲 | 状態 |
|---|---|---|
| **完全互換を確認済み** | 整数DWT（`-t 1`、実運用の既定）での全パイプライン166/166ケース／Rice・2の補数変換・DPCM(DC・AC)・パターンマッピング・`AdjustOutPut`・`CodingOptions`・`ACDepthEncoder`/`Decoder`・`DCEntropyEncoder`/`Decoder`の関数レベル全数検証9/9／ランダム化fuzz試験（CIで継続実行）／`readme_kielymods`記載の外部バグ指摘8項目／コア15ファイル全ての実効カバレッジ100.00%（行・分岐とも） | 不一致 **0件** |
