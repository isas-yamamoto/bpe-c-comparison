...
作成時点のコミット: 本リポジトリ `cef74ca` 以降 / bpe-rs `e3451cf`（bpe-rs 側はローカルのみ・未 push）。
...
|---|---|---|
| コア15ファイル全体 | **91.55%**（6705/7324行） | **82.65%**（5555/6721） |
...
`AdjustOutput.c` は §3.5 の1024ケース関数直接呼び出しにより行65.02%→96.90%、分岐56.60%→84.28%へと大幅に伸びた。全パイプライン試験（観点1）だけでは実質到達不能だったブロック内停止座標の細部まで踏めるようになったのが主因。`bpe_decoder.c`/`bpe_encoder.c`/`header.c` は直接呼び出しハーネスの対象外のため変化なし。
...
- **コア15ファイル全体の行カバレッジは91.55%（分岐82.65%）に達し、当初目標としていた90%を上回った。** これは`AdjustOutPut`の直接呼び出しハーネス追加が支配的な要因であり（同関数だけでコアアルゴリズム全体の約49%の行数を占めるため、この1関数の伸びが全体の数値を大きく押し上げた）、`bpe_decoder.c`・`header.c`のデッドコード相当分など、この先も残りやすい少数のギャップを除けば、現実的な意味でほぼ天井に近い水準にある。
