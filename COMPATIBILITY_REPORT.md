...
37ケース（うち1件は `--include-slow` 指定時のみの低頻度回帰ケース）。全ケースで **エンコード出力バイト一致・相互デコード出力バイト一致（C→Rust, Rust→C 双方向）** を確認。
...
| レート制限デコードの掃引（18ケース） | セグメントサイズ64（既定256より細かく分割）× t=1で10段階・t=0で8段階のレート値（0.05〜3.0bpp）を掃引し、`AdjustOutPut`（レート制限時の途中停止デコード）を多数の異なる停止位置で駆動する。t=0側の2値（0.1, 0.75）は既知の残存ULP差により除外（§3.3, §5参照） |
| large_segment_slow | 1セグメント33,000ブロック（`short i` オーバーフローの回帰、週次CIのみ） |

**結果: 37/37 PASS**（2026-07-25 時点）。
...
§2.1・§2.2 の試験群が、C参照実装の実コードのどれだけの範囲を実際に踏んでいるかを `gcov` で計測した（コアアルゴリズム15ファイル対象。CLI引数解析の `main.c`、サードパーティ製 `getopt.c`、エラー文言表の `errorhandle.c` は対象外）。レート制限デコード掃引（§2.1）とAC側DPCMベクタ（§2.2）追加後の値（カッコ内は追加前）。

| 対象 | 行カバレッジ | 分岐カバレッジ（両方向） |
|---|---|---|
| コア15ファイル全体 | **66.43%**（4865/7324行、旧47.75%） | **57.04%**（3834/6721、旧37.67%） |
| うち `AdjustOutput.c` を除く14ファイル | 84.65%（3170/3745行、旧81.79%） | 78.70%（2165/2751、旧76.26%） |
| `AdjustOutput.c` のみ | **47.36%**（3579行中、旧12.13%） | **42.04%**（旧10.93%） |

`AdjustOutput.c` のカバレッジがレート掃引の追加でおよそ4倍（行12.13%→47.36%、分岐10.93%→42.04%）に伸びた。まだ全体の半分強にとどまるが、「ほぼ試験できていない」状態からは大きく前進している。
...
- **レート制限時の途中停止デコード（`AdjustOutPut`）はカバレッジが改善したが（12%→47%）、まだ過半には届いていない**。この関数は停止した段階（ステージ・ビットプレーン・シンボル種別の組み合わせ）ごとに異なるコードパスを通る非常に分岐の多い実装のため、依然として引き続き手を入れるべき領域。
...
1. `AdjustOutPut` のカバレッジは12%→47%まで改善したが、まだ過半に届いていない。segment_full / bitplane / stage / symbol種別のさらに多様な組み合わせを踏むテストケース（セグメントサイズやレート値の追加バリエーション、あるいは意図的に特定の停止位置を狙う専用ケース）を追加する。
