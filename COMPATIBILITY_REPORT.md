...
101ケース（うち1件は `--include-slow` 指定時のみの低頻度回帰ケース）。全ケースで **エンコード出力バイト一致・相互デコード出力バイト一致（C→Rust, Rust→C 双方向）** を確認。
...
**結果: 101/101 PASS**（2026-07-26 時点、デコード側バイト順反転ケース追加により初回は12件FAIL — §3.6参照）。
...
|---|---|---|
| コア15ファイル全体 | **92.87%**（6802/7324行） | **84.18%**（5658/6721） |
| `AdjustOutput.c` のみ | 96.93%（変化なし） | 84.46%（変化なし） |
| `AC_BitPlaneCoding.c` のみ | **86.07%**（210/244行、直前79.92%） | **80.73%**（155/192、直前71.88%） |
| `DC_EnDeCoding.c` のみ | **88.60%**（303/342行、直前83.04%） | **80.08%**（197/246、直前70.73%） |
| `bpe_decoder.c` のみ | 87.87%（変化なし） | 79.76%（誤差程度の変化） |
| `bpe_encoder.c` のみ | 75.98%（変化なし） | 70.63%（変化なし） |
| `header.c` のみ | 78.38%（変化なし、デッドコード確定済み） | 55.56%（変化なし） |

`AdjustOutput.c` は §3.5 の1024ケース関数直接呼び出しにより行65.02%→96.90%、分岐56.60%→84.28%へと大幅に伸びた（本ラウンドではさらに誤差程度微増）。全パイプライン試験（観点1）だけでは実質到達不能だったブロック内停止座標の細部まで踏めるようになったのが主因。`bpe_decoder.c` は、§2.1に追加した float DWT×16bit画素の組み合わせ・符号付き16bit語の明示的ビット深度・デコード側バイト順反転（§3.6でRustの実バグ発見につながった）の3種の追加ケースにより行66.18%→87.87%、分岐50.00%→79.17%まで伸びた。`AC_BitPlaneCoding.c`/`DC_EnDeCoding.c`/`BPEBlockCoding.c` は本ラウンドで追加したAC係数振幅制御画像により上記の通り伸びた。`bpe_encoder.c`/`header.c` はいずれのハーネスの対象外のため変化なし。
...
- **コア15ファイル全体の行カバレッジは92.87%（分岐84.18%）に達し、当初目標としていた90%を上回った。** これは`AdjustOutPut`の直接呼び出しハーネス追加が支配的な要因であり（同関数だけでコアアルゴリズム全体の約49%の行数を占めるため、この1関数の伸びが全体の数値を大きく押し上げた）、`header.c`・`bpe_decoder.c`・AC/DC側のデッドコード相当分など、この先も残りやすい少数のギャップを除けば、現実的な意味でほぼ天井に近い水準にある。
