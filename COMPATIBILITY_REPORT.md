...
161ケース（うち1件は `--include-slow` 指定時のみの低頻度回帰ケース）。全ケースで **エンコード出力バイト一致・相互デコード出力バイト一致（C→Rust, Rust→C 双方向）** を確認。
...
結果、`StagesCodingGaggles.c` は行85.93%→88.85%・分岐81.56%→83.69%まで伸びた。副産物として `ricecoding.c`（行92.99%→95.15%・分岐88.36%→90.09%）と `AdjustOutput.c`（行96.93%→97.35%・分岐84.46%→85.16%）もわずかに伸びた。残る未実行行（65行）は、`StageStop_2Bits` 常時3固定という別のCLI到達不能フィールド（後述）・パターン長が構造上4になり得ない防御的分岐・デバッグ用の無害なno-opスキャフォールドなどの確定デッドコードと、依然として特定タイミングに依存する残りのチェックポイント（約30行、`AdjustOutPut`の残存3%と同種の性質）に分かれる。

### 最終結果

| 対象 | 行カバレッジ | 分岐カバレッジ（両方向） |
|---|---|---|
| コア15ファイル全体 | **93.42%**（6842/7324行） | **84.84%**（5702/6721） |
| `AdjustOutput.c` のみ | 97.35%（直前96.93%、誤差程度の変化） | 85.16%（直前84.46%） |
| `AC_BitPlaneCoding.c` のみ | 86.07%（変化なし） | 80.73%（変化なし） |
| `DC_EnDeCoding.c` のみ | 88.60%（変化なし） | 80.08%（変化なし） |
| `StagesCodingGaggles.c` のみ | **88.85%**（518/583行、直前85.93%） | **83.69%**（472/564、直前81.56%） |
| `ricecoding.c` のみ | **95.15%**（353/371行、直前92.99%） | **90.09%**（209/232、直前88.36%） |
| `bpe_decoder.c` のみ | 87.87%（変化なし） | 79.76%（変化なし） |
| `bpe_encoder.c` のみ | 75.98%（変化なし） | 70.63%（変化なし） |
| `header.c` のみ | 78.38%（変化なし、デッドコード確定済み） | 55.56%（変化なし） |

`AdjustOutput.c` は §3.5 の1024ケース関数直接呼び出しにより行65.02%→96.90%、分岐56.60%→84.28%へと大幅に伸びた。全パイプライン試験（観点1）だけでは実質到達不能だったブロック内停止座標の細部まで踏めるようになったのが主因。`bpe_decoder.c` は、§2.1に追加した float DWT×16bit画素の組み合わせ・符号付き16bit語の明示的ビット深度・デコード側バイト順反転（§3.6でRustの実バグ発見につながった）の3種の追加ケースにより行66.18%→87.87%、分岐50.00%→79.17%まで伸びた。`AC_BitPlaneCoding.c`/`DC_EnDeCoding.c`/`BPEBlockCoding.c` はAC係数振幅制御画像により伸びた。`StagesCodingGaggles.c`/`ricecoding.c`/`AdjustOutput.c`（微増）はセグメントサイズ多様化掃引により本ラウンドで伸びた。`bpe_encoder.c`/`header.c` はいずれのハーネスの対象外のため変化なし。
...
| BPEBlockCoding.c | 94.86% | 85.75% |
| StagesCodingGaggles.c | 88.85% | 83.69% |
| DC_EnDeCoding.c | 88.60% | 80.08% |
...
`AC_BitPlaneCoding.c`（残り13.93%）・`DC_EnDeCoding.c`（残り11.40%）— 両ファイルは同じ構造（DC側/AC側で並行実装）を持つため、同種の未到達パターンが両方に現れる:

- **`OptDCSelect`/`OptACSelect == FALSE` の「ヒューリスティック」符号選択分岐**（`ACGaggleEncoding`/`DCGaggleEncoding` 内、各20行前後）: `HeaderInilization`（header.c:47-48）が両フラグを常に `TRUE`（ブルートフォース最適選択）にハードコードし、CLIのどのオプションからも `FALSE` に変更する手段が存在しない。`header.c` の `CodewordLength_2Bits`/`CustomWtFlag` と同種の、参照実装自身のCLIが露出させていない設定項目。
...
- **コア15ファイル全体の行カバレッジは93.42%（分岐84.84%）に達し、当初目標としていた90%を上回った。** これは`AdjustOutPut`の直接呼び出しハーネス追加が支配的な要因であり（同関数だけでコアアルゴリズム全体の約49%の行数を占めるため、この1関数の伸びが全体の数値を大きく押し上げた）、`header.c`・`bpe_decoder.c`・AC/DC側・`StagesCodingGaggles.c`のデッドコード相当分など、この先も残りやすい少数のギャップを除けば、現実的な意味でほぼ天井に近い水準にある。
...
6. `bpe_encoder.c`（75.98%/70.63%）・`bitsIO.c` を除く残存最下位ファイルは現時点で全て「対応済み／確定デッドコード」に分類済み。`bpe_encoder.c` の残存49行は大半（35行）が旧式のファイルサイズ推測フォールバック（`-w`/`-h`必須の現行CLIからは到達不能と推定、未確認）で、次に手を付けるなら最有力候補だが、確認・対応は本レポート時点では未着手。
