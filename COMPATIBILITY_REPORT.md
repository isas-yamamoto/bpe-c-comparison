...
作成時点のコミット: 本リポジトリ `74547a3` 以降 / bpe-rs `a467c27`（bpe-rs 側はローカルのみ・未 push）。
...
60ケース（うち1件は `--include-slow` 指定時のみの低頻度回帰ケース）。全ケースで **エンコード出力バイト一致・相互デコード出力バイト一致（C→Rust, Rust→C 双方向）** を確認。
...
§2.1・§2.2 の試験群が、C参照実装の実コードのどれだけの範囲を実際に踏んでいるかを `gcov` で計測した（コアアルゴリズム15ファイル対象。CLI引数解析の `main.c`、サードパーティ製 `getopt.c`、エラー文言表の `errorhandle.c` は対象外）。値は signed16/pixels12/signed+float ケース追加後のもの（カッコ内は直前の値 / さらにその前 / 初期値）。

| 対象 | 行カバレッジ | 分岐カバレッジ（両方向） |
|---|---|---|
| コア15ファイル全体 | **72.25%**（5292/7324行、直前71.86%、初期47.75%） | **62.56%**（4205/6721、直前62.22%、初期37.67%） |
| `AdjustOutput.c` のみ | 57.89%（3579行中、初期12.13%） | 50.55%（初期10.93%） |
| `bpe_decoder.c` のみ | **66.18%**（直前57.72%） | **50.00%**（直前40.48%） |
| `bpe_encoder.c` のみ | 75.98%（直前73.04%） | 70.63%（直前65.08%） |
| `header.c` のみ | 78.38%（変化なし） | 55.56%（変化なし） |

`bpe_decoder.c`/`bpe_encoder.c` は signed16/pixels12/signed+float ケースの追加で伸びた。`header.c` は変化がなかった——理由は原因調査済みで、残る未実行行3件（`CodewordLength_2Bits` の非ゼロ値、`CustomWtFlag`、`UseFill=TRUE`）は**C参照実装自体のデッドコード**と確認した（詳細下記）。テストの不足ではなくCLIから到達不能な経路のため、これ以上テストケースを増やしても`header.c`のカバレッジは伸びない。
